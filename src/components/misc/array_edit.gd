...
func override_array(array: PackedStringArray) -> void:
	for item: Control in _items_vbox.get_children():
		item.hide()
		item.queue_free()
...
func add_item(value:="") -> void:
	...
	item.move_requested.connect(func(dir: int) -> void:
...
func _update_items_move_enabled() -> void:
	for item: Item in _items_vbox.get_children():
...
class Item extends HBoxContainer:
	...
	func move_up_enable(enabled: bool) -> void:
		_move_up_btn.disabled = not enabled

	func move_down_enable(enabled: bool) -> void:
		_move_down_btn.disabled = not enabled

	func get_value() -> String:
		return _line_edit.text.strip_edges()
	
	func _emit_move_requested(dir: int) -> Callable:
		return func() -> void: move_requested.emit(dir)
