...
## or derives it from project files and executes it with user arguments.


class Route extends Routes.Item:
	## Route handler for the exec command.
...
class Request:
	## Request object for the exec command.
...
func _find_editor(req: Request) -> LocalEditors.Item:
	if req.explicit: return _find_explicit(req)
...
func _find_implicit(req: Request) -> LocalEditors.Item:
	...
	return _find_editor_by_dir(current_dir, _is_upwards(req.user_args))
...
func _find_by_godot_version_file(base_dir: String) -> LocalEditors.Item:
	...
	if not FileAccess.file_exists(godot_version_file): return null
...
func _find_by_project_godot_file(base_dir: String) -> LocalEditors.Item:
	...
	if not FileAccess.file_exists(project_godot_file): return null
...
func _first_not_empty(opts: Array) -> String:
	for opt: String in opts:
		if not opt.is_empty(): return opt
	return ""
