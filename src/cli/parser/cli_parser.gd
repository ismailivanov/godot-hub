...
## supporting namespaces, verbs, flags, and positional arguments.
...
class ParsedOption:
	## Represents a parsed command-line option with its name forms and values.
...
class ParsedArguments:
	## Contains all parsed arguments including options and positional names.
	...
	func _init(names: Array[String], options: Array[ParsedOption]) -> void:
		self.names = names
		for option: ParsedOption in options:
			_options[option.long_name] = option
			_options[option.short_name] = option


	func get_first_name(default: String = "") -> String:
		var name := default
		if len(names) > 0:
			name = names.front()
		return name
	...
	func has_options(names: Array[String]) -> bool:
		for name: String in names:
			if _options.has(name): return true
		return false
...
class ParsedCommandResult:
	## The final result of parsing a command with all extracted components.
...
class CommandParser:
	## Main parser class that processes tokens using a defined grammar.
	...
	func _expect_namespace() -> String:
		if not _grammar.supports_namespace(token): return ""
	...
	func _expect_verb(scope: String) -> String:
		if not _grammar.supports_verb(scope, token): return ""
