...
## Config backed collection of user projects.
...
class Item:
	...
	var editor: LocalEditors.Item:
		get:
			if has_invalid_editor: return null
	...
	func _init(
		...
		self._local_editors.editor_removed.connect(_check_editor_changes)
		self._local_editors.editor_name_changed.connect(_check_editor_changes)
		self._local_editors.editor_brand_changed.connect(_check_editor_changes)
	...
	func editor_is(editor: LocalEditors.Item) -> bool:
		if has_invalid_editor: return false
...
class ExternalProjectInfo extends RefCounted:
	...
	var name: String:
		get: return _name
		set(value):
			if value.strip_edges().is_empty() or is_missing: return
			_name = value
			var cfg := ConfigFile.new()
			var err := cfg.load(_project_path)
			if not err:
				cfg.set_value("application", "config/name", _name)
	...
	var version_hint: String:
		...
		set(value):
			...
			var err := cfg.load(_project_path)
			if not err:
				cfg.set_value("godots", "version_hint", _version_hint)
				cfg.save(_project_path)
	...
	func _load_icon(cfg: ConfigFile) -> Texture2D:
		...
		if icon_path.begins_with("uid://"):
			...
			if uid != ResourceUID.INVALID_ID:
				...
				if uid_cache != null and uid_cache.get_error() == OK:
					...
					if entries > 0 and entries < int(file_len / 12.0) + 1:
						for i: int in entries:
							...
							# Cap at remaining file size to avoid huge allocations on corrupt files.
							if length <= 0 or uid_cache.get_position() + length > file_len:
								break
	...
	func sort_editor_options(options: Array) -> void:
		...
		var check_version_hint_similarity := func(version_hint: String) -> int:
			return VersionHint.similarity(_version_hint as String, version_hint)
